...
def test_overhead_accounting(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    if expected_overhead < 0:
        expected_overhead = 0
    assert int(costs.get("overhead_ms", -1)) == expected_overhead

    witness = json.loads((result.ucr_path.parent / "witness.json").read_text())
    witness_costs = witness.get("costs", {})
    assert isinstance(witness_costs, dict)
    assert int(witness_costs.get("overhead_ms", -1)) == expected_overhead
