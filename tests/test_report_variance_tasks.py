...
def _write_report(path: Path, runs: list[dict]) -> None:
    path.write_text(json.dumps({"runs": runs, "metrics": {}}))


def test_report_variance_tasks(tmp_path: Path) -> None:
    ...
    _write_report(
        report1,
        [
            {"task": "t1", "seed": 0, "verify_phase_ms": 10},
            {"task": "t2", "seed": 0, "verify_phase_ms": 20},
        ],
    )
    _write_report(
        report2,
        [
            {"task": "t1", "seed": 0, "verify_phase_ms": 30},
            {"task": "t2", "seed": 0, "verify_phase_ms": 21},
        ],
    )
    _write_report(
        report3,
        [
            {"task": "t1", "seed": 0, "verify_phase_ms": 50},
            {"task": "t2", "seed": 0, "verify_phase_ms": 19},
        ],
    )
    ...
    rows, _unknown, _total, missing = module._task_variance(
        reports, "verify_phase_ms", group_mode="task+seed"
    )
    assert missing == 0
