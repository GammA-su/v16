...
baseline_p = (
    ...
    if len(sys.argv) > 1
    else pathlib.Path("baselines/sealed-smoke.baseline.json")
)
...
def pass_rate(result: dict[str, float]) -> float:
    return float(result["pass_count"]) / max(1, float(result["n"]))
...
if cr + 1e-12 < max(MIN_PASS_RATE, br - 0.001):
    errs.append(
        f"pass_rate {cr:.4f} regressed vs baseline {br:.4f} or below min {MIN_PASS_RATE:.2f}"
    )
...
print(
    "baseline pass_rate="
    f"{br:.4f} current pass_rate={cr:.4f} canary_hits={c.get('canary_hits', 0)}"
)
